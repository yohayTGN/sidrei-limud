 ...
 * ============================================================================
 *
 *  Level 1, step 2/3 of the migration (DECISIONS.md #15). Plain JS, no
 *  framework, no build step — wrangler bundles this file as-is.
 *
 *  Two jobs:
 *
 ...
 *     sends a token; it only ever talks to this Worker.
 *
 ...
 *
 *  WHY text/plain to Apps Script, not application/json: Apps Script has no
 *  CORS support at all — no OPTIONS handling, no way to set response
 *  headers on TextOutput. Only "simple" requests skip a preflight: GET, or
 *  POST with Content-Type: text/plain. A JSON content-type would trigger a
 *  preflight Apps Script can never answer. The JSON itself still travels
 *  as the body either way — Api.gs's doPost parses the raw string itself,
 *  regardless of what Content-Type says. Do not "fix" this to
 *  application/json; see DECISIONS.md #15 and apps-script/Api.gs's own
 *  header comment.
 *
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET' &&
      ...
      return handleSefariaProxy(url, ctx);
    }
...
async function handleApiCall(fnName, request, env) {
  ...
  if (!env.SCRIPT_URL || !env.API_TOKEN) {
    return jsonResponse({ ok: false, error: 'ה-Worker לא הוגדר: חסר SCRIPT_URL או API_TOKEN.' }, 500);
  }
...
async function handleSefariaProxy(url, ctx) {
 ...
 * headers) שאי אפשר לקרוא ממנה Location בכלל — אבל ב-Cloudflare Workers
 * ה-fetch החלופי חושף את קוד הסטטוס וה-headers האמיתיים גם ב-manual,
 * בכוונה, בשביל התבנית הזו. זה תלוי בזמן-הריצה של Workers, לא בספק Fetch
 * הכללי — אם זה אי-פעם רץ מחוץ ל-Workers, זה יישבר בשקט (status תמיד 0).
 */
